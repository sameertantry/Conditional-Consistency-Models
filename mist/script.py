...
class PairedDataset(Dataset):
    def __init__(
        ...
        self.HE_images = sorted(os.listdir(HE_dir))
        self.IHC_images = sorted(os.listdir(IHC_dir))
    ...
    def __getitem__(self, index: int) -> Optional[Tuple[Tensor, Tensor]]:
        HE_path = os.path.join(self.HE_dir, self.HE_images[index])
        IHC_path = os.path.join(self.IHC_dir, self.IHC_images[index])

        HE_image = Image.open(HE_path).convert("RGB")
        IHC_image = Image.open(IHC_path).convert("RGB")

        if HE_image.size != IHC_image.size:
            print(f"Skipping image pair at index {index} due to mismatched sizes")
            return None

        if torch.rand(1).item() > 0.5:
            HE_image = TF.hflip(HE_image)
            IHC_image = TF.hflip(IHC_image)

        i, j, h, w = T.RandomCrop.get_params(HE_image, output_size=self.crop_size)
        HE_image = TF.crop(HE_image, i, j, h, w)
        IHC_image = TF.crop(IHC_image, i, j, h, w)
...
class ImageDataModuleConfig:
    ...
    image_size_crop: Tuple[int, int] = (256, 256)  # Size for random cropping
    image_size_resize: Tuple[int, int] = (128, 128)  # Resize to 128x128
    batch_size: int = 2 # Number of images in each batch
...
class LLVIPDataModule(LightningDataModule):
    ...
    def setup(self, stage: str = None) -> None:
        ...
        self.dataset = PairedDataset(
            HE_dir=os.path.join(self.config.data_dir, "HE/train"),
            IHC_dir=os.path.join(self.config.data_dir, "IHC/train"),
...
class LitImprovedConsistencyModel(LightningModule):
    ...
    def training_step(self, batch: Union[Tensor, List[Tensor]], batch_idx: int) -> None:
        ...
        HE_images, IHC_images = batch  # Unpack the batch
...
class TrainingConfig:
    ...
    trainer: Trainer
    model_ckpt_path: str = "checkpoints/mist_her2"
    seed: int = 42
...
def run_training(config: TrainingConfig) -> None:
    ...
    #print(dm.dataset.HE_images[:5])
    #print(dm.dataset.IHC_images[:5])
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints_mist_her2",
        filename="{epoch}-{step}",
        save_top_k=-1,  # Save all checkpoints
        every_n_epochs=1,  # Adjust as needed
    )
    # Set up the logger
    logger = TensorBoardLogger("logs", name="mist_her2_icm")

    training_config = TrainingConfig(
        image_dm_config=ImageDataModuleConfig(data_dir="datasets/mist_her2"),
        unet_config=UNetConfig(),
