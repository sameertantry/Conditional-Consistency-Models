...
def model_forward_wrapper(
    ...
    **kwargs : Any
    ...
    return c_skip * x + c_out * model(x, sigma, v, **kwargs)
...
class ImprovedConsistencyTraining:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        # Add noise to infrared images
        next_noisy_x = x + pad_dims_like(next_sigmas, x) * noise
        # Concatenate with visible images
        # input_next = torch.cat([next_noisy_x, v], dim=1)
        ...
        with torch.no_grad():
            current_noisy_x = x + pad_dims_like(current_sigmas, x) * noise
            # input_current = torch.cat([current_noisy_x, v], dim=1)
