...
metrics_file = "metrics_lolv2_real.txt"                                          # File to save PSNR and SSIM metrics
...
def get_infrared_image_name(visible_image_name):
    ...
    if visible_image_name.lower().startswith(prefix):
        # Find the length of the prefix to replace it accurately
        prefix_length = len(prefix)
        # Replace the prefix with 'normal', preserving the original case after the prefix
        infrared_image_name = replacement + visible_image_name[prefix_length:]
        return infrared_image_name
    else:
        raise ValueError(f"Unexpected visible image prefix in '{visible_image_name}'. Expected to start with '{prefix}'.")
...
def process_image(visible_image_name):
    ...
    infrared_image_path = os.path.join(infrared_folder, infrared_image_name)
